...
def linearPath(curPos, curOri, desPos, desOri, duration, numDots):  # respect to the center point
    ...
    durationPer = int(duration / numDots)  # time between each point
    ...
    deltaXYZ = (desBox - curBox) / numDots  # return a 8*3 of delta needed per step
    deltalength = np.zeros((numDots, 8))
    for n in range(numDots):  # for each dot
        for i in range(8):  # for each boxpoint
            deltalength[n][i] = p2plength(curBox[i] + ((n + 1) * deltaXYZ[i]), pulley[i]) - \
                                p2plength(curBox[i] + (n * deltaXYZ[i]), pulley[i])  # find delta cable length
    print(deltalength)
...
numDots = 5
# call the func
linearPath(curPos, curOri, desPos, desOri, duration, numDots)
