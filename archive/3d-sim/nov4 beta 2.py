...
def coord2box(desPos, desOri):
    ...
    # Time rotational matrix and box Point to find the new box point
    newboxPoint = np.matmul(boxPoint, rotMat)
    boxPoint = newboxPoint
    print(boxPoint)


    # add old boxPoint with change of boxpoint to get new boxpoint
    boxPoint = np.add(boxPoint, desPos)
    return boxPoint
...
def input_test():
    # use global variable and return a desOri angle respect to three axi
    global desOri
