...
def linearPath(curPos, curOri, desPos, desOri, duration):
    ...
    # Calculate the positions of boxpoints
    curBox = coord2box(curPos, curOri)
    desBox = coord2box(desPos, desOri)

    for i in range(8):  # for eight cable
        deltaPos = p2plength(desBox[i], curBox[i])
        a = curBox[i]  # temp a for the current box position
        for n in range(numDots):  # for each dots
            b = (desBox[i] - a) / (n+1) #
            dist = p2plength(b, a)
            # Update Output
            a = b
