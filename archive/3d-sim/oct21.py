...
def spin(boxPoint, desOri):  # This works fine unless you use multiple of them... What is that about?
    for i in range(len(boxPoint)):
        # roll
        if desOri[0] != 0:
            r = m.hypot(boxPoint[i][1], boxPoint[i][2])  # Find the radius
            theta = m.atan(abs((boxPoint[i][2])) / abs((boxPoint[i][1])))  # Find the theta in 1st quadrant
            if (boxPoint[i][1] < 0) and (boxPoint[i][2] > 0):  # Adjust theta based on the real quadrant
                theta += m.pi / 2
            elif (boxPoint[i][1] < 0) and (boxPoint[i][2] < 0):
                theta += m.pi
            elif (boxPoint[i][1] > 0) and (boxPoint[i][2] < 0):
                theta += 3 * m.pi / 2
            boxPoint[i][1] = r * m.cos(theta + desOri[0])  # Update boxPoint Location
            boxPoint[i][2] = r * m.sin(theta + desOri[0])
        # pitch
        if desOri[1] != 0:
            r = m.hypot(boxPoint[i][0], boxPoint[i][2])
            theta = m.atan(abs((boxPoint[i][2])) / abs((boxPoint[i][0])))
            if (boxPoint[i][0] < 0) and (boxPoint[i][2] > 0):
                theta += m.pi / 2
            elif (boxPoint[i][0] < 0) and (boxPoint[i][2] < 0):
                theta += m.pi
            elif (boxPoint[i][0] > 0) and (boxPoint[i][2] < 0):
                theta += 3 * m.pi / 2
            boxPoint[i][0] = r * m.cos(theta + desOri[1])
            boxPoint[i][2] = r * m.sin(theta + desOri[1])
        # yaw
        if desOri[2] != 0:
            r = m.hypot(boxPoint[i][0], boxPoint[i][1])
            theta = m.atan(abs((boxPoint[i][1])) / abs((boxPoint[i][0])))
            if (boxPoint[i][0] < 0) and (boxPoint[i][1] > 0):
                theta += m.pi / 2
            elif (boxPoint[i][0] < 0) and (boxPoint[i][1] < 0):
                theta += m.pi
            elif (boxPoint[i][0] > 0) and (boxPoint[i][1] < 0):
                theta += 3 * m.pi / 2
            boxPoint[i][0] = r * m.cos(theta + desOri[2])
            boxPoint[i][1] = r * m.sin(theta + desOri[2])
